...
def unet(input_size = (1080, 1920, 3)):
    ...
    conv1 = Conv2D(8, 3, activation = 'relu', padding = 'same', kernel_initializer =
            'he_normal')(reshaped)
...
def fcnn_functional(image_shape, n_classes):
    ...
    c1 = Conv2D(filters=16, kernel_size=(3,3), activation='relu', padding='same')(x)
    mp1 = MaxPooling2D(pool_size=2, strides=(2, 2))(c1)

    c2 = Conv2D(filters=32, kernel_size=(3,3), activation='relu', padding='same')(mp1)
    mp2 = MaxPooling2D(pool_size=2, strides=(2, 2))(c2)

    c3 = Conv2D(filters=64, kernel_size=(3,3), activation='relu', padding='same')(mp2)
    mp3 = MaxPooling2D(pool_size=2, strides=(2, 2))(c3)
    
    last_conv = Conv2D(filters=128, kernel_size=(3,3), activation='relu', padding='same')(mp3)
    out_shp = c2.shape
    u1 = Conv2DTranspose(filters=128, kernel_size=(3, 3), strides=(2, 2))(last_conv)
    ...
    u2 = Conv2DTranspose(filters=64, kernel_size=(3, 3), strides=(2, 2))(u2)
    ...
    u3 = Conv2DTranspose(filters=32, kernel_size=(3, 3), strides=(2, 2))(c4)
...
if __name__ == '__main__':
    ...
    print("Acc (pixelwise): {}, IoU: {}".format(acc, iou))
